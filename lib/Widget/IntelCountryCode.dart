...
class IntlPhoneCustomField extends StatefulWidget {
  ...
  /// & pick dialog
  final PickerDialogCustomStyle? pickerDialogStyle;

  IntlPhoneCustomField({
    Key? key,
...
class _IntlPhoneFieldState extends State<IntlPhoneCustomField> {
  ...
  void initState() {
    ...
    if (widget.initialCountryCode == null && number.startsWith('+')) {
      ...
      number = number.substring(widget.selectedCountry!.dialCode.length);
    } else {
      widget.selectedCountry = _countryList.firstWhere(
  ...
  Future<void> _changeCountry() async {
    ...
    await showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (ctx, setState) => CountryPickerCustomDialog(
  ...
  Widget build(BuildContext context) {
    return TextFormField(
      ...
      decoration: widget.decoration.copyWith(
        ...
        counterStyle: TextStyle(color: AppColors.WHITE, fontSize: 12.sp),
      ...
      onSaved: (value) {
        widget.onSaved?.call(
          PhoneNumber(
            countryISOCode: widget.selectedCountry!.code,
            countryCode: '+${widget.selectedCountry!.dialCode}',
            number: value!,
          ),
        );
      },
      ...
      onChanged: (value) async {
        ...
        else if (widget.autovalidateMode != AutovalidateMode.disabled) {
          msg = widget.disableLengthCheck ||
              value.length >= widget.selectedCountry!.minLength &&
                  value.length <= widget.selectedCountry!.maxLength
              ? null
              : (widget.invalidNumberMessage ?? null);
      ...
      validator: widget.isValidate
        ...
        if (value!.isEmpty) {
          return "       "+"Please Enter Mobile Number";
            ...
            value.length > widget.selectedCountry!.maxLength) &&
            value.length < 10 /*|| !value.trim().isValidMobile()*/) {
      ...
      maxLength:
          widget.disableLengthCheck ? null : widget.selectedCountry!.maxLength,
